...
b. tech debt
* ReqresResponse.LogTrace: it would be good to be able to output the request and the request took as well as the RawResponse.  FiddlerCore could potentially be used for this.  Assuming the tests are run several times a day, doing this for every successful run would take up a lot of space, so it would be best to do this either when the test fails or when explicitly set to verbose mode.  It would be really useful to get this data to show up in the ReportUnit page.
* Another nice thing to do with the length of time the request took to complete would be to output it to a csv file and then graph response times.

2. Selenium Tests
a. As I mentioned when we met, I've not done very much work with Selenium - a month or two's work about 5 or 6 years ago.  It was as fun to do as I remembered but I ran into problems trying to click in the mouse over (quick view) and the lightbox (continue shopping/proceed to checkout).
...
b. There seems to be an issue with the InternetExplorer driver.  I call webdriver.quit in a cleanup method on the page factory, and this closes down the driver and the browser of chrome and firefox.   It doesn't close down the browser for IE though, which is a bit of a pain.  Google brings up quite a few people with the same problem.  I've not put IE in the tests for this reason, but it would need to be sorted out. 
...
The project will need to be built before RunTests.bat will work.